    ...
    def get_mail_content(self, maillist, fetching_count, result_count, service, user):
        ...
        for i in range(fetching_count):
            ...
            try:
                ...
                # wordcloud
                col5, col6, col7, col8, col9 = self.st.columns((1,0.3,2,0.3,5))

                # wordcloud - mail_from
                with col5:
                    self.st.write("---")
                    self.st.write("● mail_from")
                    self.visualizationTool.wordcloud(mail_from, 300, 10)

                # wordcloud - mail_subject
                with col7:
                    self.st.write("---")
                    self.st.write("● mail_subject")
                    self.visualizationTool.wordcloud(mail_subject, 400, 30)

                # wordcloud - mail_snippet
                with col9:
                    self.st.write("---")
                    self.st.write("● mail_snippet")
                    self.visualizationTool.wordcloud(mail_snippet, 480, 200)
