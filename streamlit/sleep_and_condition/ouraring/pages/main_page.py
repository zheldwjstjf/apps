...
class MainPage:
    ...
    def main_page(self, sleep, start_date, end_date, options1, options2, options3, key_word_list1, key_word_list2, key_word_list3):
        ...
        if (start_date!=None) and (end_date!=None):
            ...
            sleep_str.replace("score_deep", "熟睡")
            sleep_str.replace("score_disturbances", "睡眠コントリビューター")
            sleep_str.replace("score_efficiency", "睡眠効率")
            sleep_str.replace("score_latency", "入眠潜時")
            sleep_str.replace("score_rem", "レム睡眠")
